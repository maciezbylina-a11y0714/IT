<?php   
    ...
    if (empty($receiver_email)) {
        error_log("RECEIVER_EMAIL environment variable is not set!");
        echo '<center><h1>Error: Email configuration missing. Please contact the administrator.</h1></center>';
        exit;
    }
    ...
    if (empty($mail_username) || empty($mail_password)) {
        error_log("MAIL_USERNAME or MAIL_PASSWORD environment variables are not set!");
        echo '<center><h1>Error: Email server configuration missing. Please contact the administrator.</h1></center>';
        exit;
    }
